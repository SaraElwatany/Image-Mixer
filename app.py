...
def calculate(ph,mg):
    print(mg,ph)
    img= ImageProcessing.combined_mask(mg,ph)
    return img
...
def upload_image():
    ...
    imageB = ImageProcessing(imgs[0], freq_b , magnitude_b, phase_b)

    phases.append(phase_a) 
    magnitudes.append(magnitude_a)
    ...
    if options[2] == "select":
        if len(phases)!=0 and len(magnitudes)!=0:
             ImageProcessing.combined_mask(magnitudes[len(magnitudes)-1],phases[len(phases)-1])
    ...
    if option1== "2":
            if no_boxes1==2:
                ...
                if value1== "AND": 
                    ...
                    mag= Image.mask("static/Images/magnitude1.png",11,"magnitude",img1_box1.get(img1_indx1),img1_box1.get(img1_indx2),img1_box1.get(img1_indx3),img1_box1.get(img1_indx4))
                    magnitudes.append(mag)
        ...
        if no_boxes1==2:
            ...
            if value1== "AND":
                ...
                phases.append(phase)
        if no_boxes1==1: 
                phase=Image.mask("static/Images/phase1.png",11,"phase",img1_box1.get(img1_indx1),img1_box1.get(img1_indx2),img1_box1.get(img1_indx3),img1_box1.get(img1_indx4))
                phases.append(phase)
...
def boxes2():
    global no_boxes2  
    output= request.get_json()
    result3= json.loads(output)         #this converts the json output to a python dictionary
    indx= list(result3.keys())[0]
    no_boxes2= result3.get(indx)
    ...
    if option2== "1":
        if no_boxes2==2:
            ...
            if value1== "AND":
                ...
                phases.append(phase)
        if no_boxes2==1: 
                phase=Image.mask("static/Images/phase2.png",21,"phase",result2.get(img2_indx1),result2.get(img2_indx2),result2.get(img2_indx3),result2.get(img2_indx4))
                phases.append(phase)
        ...
        if no_boxes2==2:
            ...
            mag= Image.mask("static/Images/magnitude2.png",21,"magnitude",result2.get(img2_indx1),result2.get(img2_indx2),result2.get(img2_indx3),result2.get(img2_indx4))
            magnitudes.append(mag)
